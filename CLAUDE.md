...
- **계획·설계·진단 = Opus 5.5**(`model: opus`): `feature-architect`(아키텍처 분기·TDD 플랜·골든 시나리오 — 계획/설계)와 `code-auditor`(intent-review·SSOT·보안 감사 — 진단). 분기 결정과 결함 진단은 추론 품질이 결과를 좌우하므로 최상위 모델.
