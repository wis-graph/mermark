...
- **Vault image attachments** (`image.attach`): in a document that belongs to a registered vault, pick an image — if it's already inside that vault, nothing is copied, it's just inserted as `![[name.ext]]`; otherwise it's copied atomically into `.attachments/` (no-clobber, never overwrites an existing file) before being inserted the same way. `![[name]]` resolves by searching the OWNING vault's root by name (shallowest match first, then path order) — the search root is the document's own location, never whichever vault happens to be active.
