...
//! only mints it via `crypto_token::mint_token`, reusing that CSPRNG-backed
...
/// source `crypto_token::mint_token` uses for its token bytes) rather than
...
pub fn redeem(state: &mut PairingState, offered: &str, now_ms: u64) -> Result<String, PairError> {
    ...
    state.used = true;
    Ok(crate::crypto_token::mint_token())
}
...
async fn pair_handler(
    ...
    let id = crate::crypto_token::mint_token();
