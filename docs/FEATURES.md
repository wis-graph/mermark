...
- **웰컴 대시보드** — 파일 인자 없이 실행 시 에디터 대신 표시(`src/welcome/welcome-pane.ts`의 `createWelcomePane`, 주입식 — main.ts 인라인에서 모듈 추출). 즐겨찾기·최근 문서 카드 그리드 + **빈 상태 CTA**: 둘 다 비어 있으면 "폴더 열기" 버튼(기존 explorer 열기 재사용 — 신규 IPC 0) + 단축키 힌트(⌘B, `effectiveBinding` 파생이라 사용자 재바인딩 반영). 최근 문서 항목의 보조 경로 행은 **이름==경로일 때 생략**(`redundantPathLabel`(chrome/path-label.ts) — recent-panel·favorites-panel과 공유하는 단일 규칙).
