...
| R9 | 좌측 사이드바 패널 | `src/sidebar/registry.ts` `registerSidebarPanel` | **런타임 ✅** — install 후 등록도 즉시 mount+배제+rehome(늦은/확장 등록이 installLeftGroupRehoming의 고정 배열 한계를 물려받지 않음). 계약: id never-rename(향후 "마지막 열린 패널 복원" 등 영속 상태 키가 될 수 있음) · `aside.hidden`이 열림/닫힘 SSOT · 버튼 순서 = 등록 순서 · **unregister 없음**(explorer/outline/recent 세 내장 패널 모두 teardown이 없으므로 — 배열만 splice하고 DOM/옵저버를 남기는 unregister는 거짓말 API). 상호배제가 이제 **N-way**(이전엔 main.ts의 3-way 하드코딩이라 4번째 패널을 등록해도 배제에 참여 못 하고 다른 패널 위에 겹쳐 떴다 — R9의 존재 이유) |
| R10 | (결번) | — | 파일 서비스 파사드(범용 read-bytes IPC 등)로 잠정 예약됐으나 R11이 신규 IPC 없이 asset 프로토콜로 해결해 보류(§ R11 실측 참고) — 필요해지면 그때 채운다 |
| R11 | 뷰어 (비md 파일 오버레이) | `src/chrome/viewer/registry.ts` `registerViewer` | **런타임 ✅** — 조회(`viewerFor`)가 렌더 시점 평가라 부팅 등록이면 즉시 반영. 계약: id never-rename · extensions 소문자·점 없음(포맷 위반 시 throw) · **확장자 충돌은 first-claim-wins**(R3의 block-feature 디스패치와 동일 의미론, throw 아님) · **unregister 없음**(YAGNI — 뷰어는 `close()`가 있어 R9와 달리 "불가능"이 아니라 "수요 부재") · don't-stack 오버레이 슬롯(`openViewer`)은 레지스트리가 아니라 **main.ts 소유**(순수 카탈로그 유지 — God-object화 회피, R9와 동일 판단). 신규 IPC 0 — `fetch(convertFileSrc(abs)).arrayBuffer()`가 이미 열린 `connect-src`/`assetProtocol.scope: ["**"]`로 성립(§ 아래). 첫 실물 확장(Excel, `src/extensions/excel-viewer/`)이 `../api` 파사드만 거쳐 이 레지스트리에 등록된다. **SheetJS는 npm 레지스트리가 아니라 공식 CDN 타르랩에서 받는다** — npm의 `xlsx`는 0.18.5에서 방치된 구버전으로 알려진 prototype-pollution/ReDoS 취약점이 있고 SheetJS가 배포를 자사 CDN으로 옮겼다(`package.json`: `"xlsx": "https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz"`, Apache-2.0). 사유는 여기와 `excel-viewer/index.ts`의 import 지점 주석 두 곳에 박제(다음 사람이 npm으로 되돌리는 것을 막기 위해). |
...
- **R11이 `readLocalFileBytes`를 파사드에 노출한다** (`src/chrome/viewer/file-bytes.ts`,
  `../api`를 통해 `src/extensions/**`에 공개) → 확장은 임의 경로의 로컬 파일을
  **바이트로 읽을 수 있다**(assetProtocol scope `**`, 신규 IPC 아님 — 위 `fetch(convertFileSrc(...))`
  경로 그대로). 탐색기가 요구했던 것(`list_dir` 펜스 없음 + `read_file` + `open_path`)보다는
  좁다 — **읽기 전용, 쓰기·디렉터리 탐색·창 스폰 없음.** 개인용 신뢰 모델("내 코드만
  로드")에서는 수용하지만, **남의 확장을 하나라도 받는 날 이것은 전체 파일 읽기 권한이다.**
...
1. §6.1을 사용자 고지로: "플러그인 활성화 = mermark가 여는 모든 파일에 대한
   전권 위임" 문장을 설치/활성화 UI에 그대로 노출. R11의 `readLocalFileBytes`도
   이 고지에 명시 포함(§6.1 — "임의 로컬 파일을 바이트로 읽을 수 있다").
