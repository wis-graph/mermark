...
describe("openHwpViewer: close (T4)", () => {
  ...
  it("close() unsubscribes the fontScale sink — a post-close zoom change no longer touches the (removed) pages container", async () => {
    ...
    expect(pagesEl.style.getPropertyValue("--hwp-page-width")).toBe("780px");
...
describe("openHwpViewer: zoom (T6, design §4.2)", () => {
  it("page width reflects fontScaleSetting on open and on every change (600px baseline × scale)", async () => {
    ...
    const pagesEl = document.querySelector(".hwp-viewer-pages") as HTMLElement;
    expect(pagesEl.style.getPropertyValue("--hwp-page-width")).toBe("900px");
