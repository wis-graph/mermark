...
describe("viewer size envelope (style contract — no viewer reinvents its own px cap)", () => {
  ...
  const VIEWER_SIZE_PREFIX_RE = /(^|[\s,>~+])\.(viewer-|image-viewer-|excel-viewer|html-viewer|hwp-viewer|docx-viewer|code-viewer)/;
...
describe("viewer content roots declare no size envelope (shell .viewer-panel is the sole owner)", () => {
  ...
  const CONTENT_ROOT_SELECTORS = [".pdf-viewer", ".html-viewer", ".excel-viewer", ".hwp-viewer", ".docx-viewer", ".code-viewer"];
  ...
  it("extension-injected CSS: .pdf-viewer/.html-viewer/.excel-viewer/.docx-viewer declare no width/height/max-width/max-height", () => {
    const files = [
      ...
      join(ROOT, "src", "extensions", "code-viewer", "index.ts"),
    ];
